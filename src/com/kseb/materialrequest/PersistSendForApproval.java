...
@WebServlet("/persistsendforapproval")
public class PersistSendForApproval extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) {
		try {
			...
			pstmt.setString(2, "Sent for Approval");
			pstmt.setDate(3, new Date(System.currentTimeMillis()));
			pstmt.setString(4, "MaterialManager");
			pstmt.setString(5, request.getParameter("statusdescription"));
			pstmt.executeUpdate();

			out.print("<script>alert('Material Request send for Approval')</script>");
